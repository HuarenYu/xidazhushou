$(function() {
	...
	function loadItem(p, s) {
		loding = true;
		$.post('/shop/menu/' + sid, {page:p, size:s}, function(data, status) {
			if (status === 'success') {
				if (data.length < 1) {
					loadFinish = true;
					$('.finish').text('暂无更多');
				...
				for (var i = 0; i < data.length; i++) {
					var tmp = format(template, 
						[data[i].name, data[i].price, data[i].id]);
					$('#itemList').append(tmp);
				}
				++page;
	...
	*/
	var template = '<tr>'+
		              '<td>{{0}}</td>'+
		              '<td>{{1}}元</td>'+
		              '<td><button class="btn btn-small" data-target="order" data-item=\'{"itemId":{{2}}}\'>添加</button></td>'+
		            '</tr>';
